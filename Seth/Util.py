...
def getNullPercents(df: pd.DataFrame, ascending=True):
    ...
    result = result[result['Null Percent'] > 0]
    result.sort_values(by='Null Percent', ascending=ascending, inplace=True)
    return result

def removeNullColumns(df: pd.DataFrame, threshold: int):
    percents = getNullPercents(df, False)
    columnsToRemove = percents[percents['Null Percent'] > threshold]
    return df.drop(columns=columnsToRemove['Column'], axis=1)
